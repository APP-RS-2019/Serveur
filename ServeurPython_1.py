...
class ThreadClient(Thread):
    ...
    def run(self):
        ...
        while ((testFinServeur == False) and ( self.testFinClient == False )) :
            try:
                self.connexion.send(("Veuillez entrer le destinataire :").encode())
                destinataire=self.connexion.recv(1024).decode()
                self.connexion.send(("Veuillez entrer le message a envoyer a "+destinataire +" :").encode())
                ...
                if destinataire == "Serveur" :
                    ...
                    try:
                        ...
                        self.connexion.send(("Le destinataire n'existe pas ou n'est pas connecte").encode())
...
while testFinServeur == False:
    try:
        ...
        connexion.send(("Veuillez entrer votre nom :").encode()) # Automatique pour les robots "
